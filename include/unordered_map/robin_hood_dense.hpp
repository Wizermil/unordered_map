...
namespace wiz::robin_hood {
    ...
    private:
        ...
        class WIZ_HIDDEN _iterator {
            friend class raw_flat_dense_hash_map;
    ...
    public:
        raw_flat_dense_hash_map() noexcept
        ...
        , _capacity{0ul}
        , _size{0ul} {}
        explicit raw_flat_dense_hash_map(size_type new_cap)
        : raw_flat_dense_hash_map() {
            rehash(new_cap);
        }
        template <typename InputIt>
        raw_flat_dense_hash_map(InputIt first, InputIt last, size_type new_cap = 0)
        : raw_flat_dense_hash_map(new_cap == 0ul ? _recommended_capacity(static_cast<usize>(last - first)) : new_cap) {
            insert(first, last);
        }
        raw_flat_dense_hash_map(std::initializer_list<value_type> il, size_type new_cap = 0ul)
        : raw_flat_dense_hash_map(new_cap == 0ul ? _recommended_capacity(il.size()) : new_cap) {
            insert(il.begin(), il.end());
        }
        raw_flat_dense_hash_map(raw_flat_dense_hash_map const& other)
        : raw_flat_dense_hash_map(_recommended_capacity(other._size)) {
            insert(other.begin(), other.end());
        }
        raw_flat_dense_hash_map(raw_flat_dense_hash_map&& other) noexcept
        : _metas{other._metas}
        ...
        ~raw_flat_dense_hash_map() { _destroy(); }

        raw_flat_dense_hash_map& operator=(raw_flat_dense_hash_map const& other) {
        ...
        raw_flat_dense_hash_map& operator=(raw_flat_dense_hash_map&& other) noexcept {
            if (WIZ_UNLIKELY(this == addressof(other))) {
                return *this;
            }
            _destroy();
        ...
        raw_flat_dense_hash_map& operator=(std::initializer_list<value_type> ilist) {
            clear();
        ...
        const_iterator begin() const noexcept { return const_cast<raw_flat_dense_hash_map*>(this)->begin(); }
        ...
        iterator find(key_type const& key) {
            ...
            __builtin_prefetch(_metas + index);
        ...
        const_iterator find(key_type const& key) const { return const_cast<raw_flat_dense_hash_map*>(this)->find(key); }
        ...
        convertible_to_iterator erase(const_iterator pos) {
            ...
            for (usize next = current + 1ul; !details::is_at_desired_position(*(_metas + next)); ++current, ++next) {
                ...
                __builtin_prefetch(_metas + next + 1ul);
            }
        ...
        convertible_to_iterator erase(const_iterator first, const_iterator last) {
            ...
            usize ind_end{last._inner._dib == nullptr ? _capacity : static_cast<usize>(last._inner._dib - _metas)};
            usize current{first._inner._dib == nullptr ? _capacity : static_cast<usize>(first._inner._dib - _metas)};
            ...
            for (; current != ind_end; ++current) {
                ...
                __builtin_prefetch(_metas + current + 1ul);
            }
        ...
        void swap(raw_flat_dense_hash_map& other) {
            wiz::swap(_metas, other._metas);
            wiz::swap(_values, other._values);
            wiz::swap(_capacity, other._capacity);
        ...
        constexpr size_type bucket_count() const {
            if (WIZ_LIKELY(_capacity != 0ul)) {
                return _capacity >> BIT_OFFSET;
            } else {
                return 0ul;
            }
        }
        ...
        void rehash(size_type new_cap) {
            if (new_cap > _capacity) {
                _grow(wiz::max(16ul, new_cap));
            }
        }

        friend bool operator==(raw_flat_dense_hash_map const& lhs, raw_flat_dense_hash_map const& rhs) {
        ...
        friend bool operator!=(const raw_flat_dense_hash_map& lhs, const raw_flat_dense_hash_map& rhs) { return !(lhs == rhs); }
    ...
    private:
        ...
        WIZ_HIDE_FROM_ABI constexpr usize _recommended_capacity(usize size) const noexcept { return size; }

        WIZ_HIDE_FROM_ABI constexpr inline u64 _moremur(u64 x) const noexcept WIZ_DISABLE_UBSAN_UNSIGNED_INTEGER_CHECK {
            x ^= x >> 27u;
...
//            usize const bucket_num{(hash >> BIT_OFFSET) & _capacity};
//            return (bucket_num << BIT_OFFSET) + (hash & MASK);
            //            usize const bucket_num{hash & _capacity};
            //            return (bucket_num << BIT_OFFSET) + static_cast<usize>((static_cast<u64>(hash) * 11400714819323198485ull) >> 60ull);
            return static_cast<usize>((static_cast<u128>(_moremur(hash)) * (wiz::max(static_cast<usize>(SEEK_MAX), _capacity) - SEEK_MAX)) >> 64u);
        }
        ...
        WIZ_HIDE_FROM_ABI pair<iterator, bool> _emplace_new(s8 distance_from_desired, usize index, Key&& key, Args&&... args) noexcept {
            if (WIZ_LIKELY(details::is_empty_or_deleted(*(_metas + index)))) {
                ...
                _grow(wiz::max(16ul, ((_capacity << 1ul) + _capacity) >> 1ul));
        ...
        WIZ_HIDE_FROM_ABI inline void _grow(size_type new_cap) noexcept {
            assert(new_cap > 0ul);
            new_cap = details::next_aligned(new_cap + SEEK_MAX - 1ul, MASK);
            ...
            wiz::swap(_capacity, new_cap);
            _size = 0ul;
            if (new_cap != 0ul) {
                for (usize index{0ul}, last{new_cap}; index < last; ++index) {
    ...
    using node_dense_hash_map = raw_flat_dense_hash_map<K, T, Hash, KeyEqual, details::node_policy<K, T, Hash, KeyEqual>>;

    template <typename K, typename T, typename Hash = wiz::hash<K>, typename KeyEqual = wiz::equal_to<K>>
    using flat_dense_hash_map = raw_flat_dense_hash_map<K, T, Hash, KeyEqual, details::policy<K, T, Hash, KeyEqual>>;
