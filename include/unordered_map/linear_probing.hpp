...
namespace wiz::linear_probing {
    ...
    public:
        ...
        iterator find(key_type const& key) {
            ...
            s8 distance{0};
            for (usize index{_index_for_hash(hash)}; distance < static_cast<s8>(VALUE_PER_BUCKET << 1ul) && !details::is_end(*(_metas + index)); ++distance, ++index) {
                if (details::is_full(*(_metas + index)) && *(_metas + index) == h2 && policy::equal(key, _values + index)) {
        ...
        float load_factor() const {
            if (WIZ_LIKELY(_is_init())) {
                return static_cast<float>(_size) / ((_capacity_minus_one + 1ul) * VALUE_PER_BUCKET);
            } else {
                return 1.f;
            }
        }
...
//            return hash & (((_capacity_minus_one + 1) << BIT_OFFSET) - 1);
            return (hash & _capacity_minus_one) << BIT_OFFSET;
        }
        ...
        WIZ_HIDE_FROM_ABI WIZ_NOINLINE pair<iterator, bool> _emplace(Key&& key, Args&&... args) {
            ...
            usize index_empty{(_capacity_minus_one + 1) << BIT_OFFSET};
            s8 distance{0};
            for (; distance < static_cast<s8>(VALUE_PER_BUCKET << 1ul) && !details::is_end(*(_metas + index)); ++distance, ++index) {
                if (details::is_full(*(_metas + index)) && *(_metas + index) == h2 && policy::equal(key, _values + index)) {
                    return {_iterator_at(index), false};
                } else if (details::is_empty_or_end(*(_metas + index))) {
                    index_empty = min(index, index_empty);
                }
                __builtin_prefetch(_metas + index + 1ul);
            }

            return _emplace_new(distance, h2, index_empty, wiz::forward<Key>(key), wiz::forward<Args>(args)...);
        }
        ...
        WIZ_HIDE_FROM_ABI pair<iterator, bool> _emplace_new(s8 distance_from_desired, s8 h2, usize index, Key&& key, Args&&... args) noexcept {
            ...
            } else if (details::is_end(*(_metas + index)) || distance_from_desired == static_cast<s8>(VALUE_PER_BUCKET << 1ul)) {
                if (WIZ_LIKELY(_is_init())) {
